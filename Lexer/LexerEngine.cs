...
namespace Lexer
{
    ...
    public class LexerEngine : ILexer<Token>
    {
        ...
        static LexerEngine()
        {
            ...
            M %= (symbol + less + symbol).With(p => Nfa.MultiOr(p[0].Text[0], p[2].Text[0], _level));
            M %= (symbol + M).With(p => Nfa.Or(new Nfa(p[0].Text[0], _level), p[1], _level));
            M %= (symbol + less + symbol + M).With(p => Nfa.Or(Nfa.MultiOr(p[0].Text[0], p[2].Text[0], _level), p[3], _level));
        }
